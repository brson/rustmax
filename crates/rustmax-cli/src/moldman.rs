use rmx::prelude::*;
use rmx::{reqwest, serde_json, tempfile::TempDir, toml, xshell};
use std::env;
use std::fs;
...
pub fn install() -> AnyResult<()> {
    ...
    if !check_clang_available() {
        bail!(
            "clang is required for mold integration but not found in PATH. Please install clang first."
        );
    }
    ...
    let tarball_path =
        download_release_tarball(temp_path, &url).context("Failed to download mold")?;
    let extracted_dir = extract_tarball(&tarball_path).context("Failed to extract mold")?;

    // Install the binary
    let mold_binary =
        find_mold_binary(&extracted_dir).context("Failed to find mold binary in archive")?;
...
pub fn update() -> AnyResult<()> {
    ...
    match current_version {
        ...
        Err(_) => {
            println!(
                "Mold not currently installed, installing {}",
                latest_version
            );
        }
    }
...
pub fn uninstall() -> AnyResult<()> {
    ...
    let config_removed =
        remove_mold_from_cargo_config().context("Failed to clean up cargo configuration")?;
    if config_removed {
        ...
        Ok(latest) => {
            if binary_installed {
                match get_installed_version() {
                    ...
                    Ok(current) => {
                        println!("  Status: Update available ({} -> {})", current, latest)
                    }
...
fn find_mold_binary(extracted_dir: &Path) -> AnyResult<PathBuf> {
    ...
    let common_paths = ["bin/mold", "mold"];
...
fn get_installed_version() -> AnyResult<String> {
    ...
    let version_output =
        String::from_utf8(output.stdout).context("mold --version output is not valid UTF-8")?;
...
fn check_cargo_config_status(config_path: &Path) -> AnyResult<CargoConfigStatus> {
    ...
    let config_content =
        fs::read_to_string(config_path).context("Failed to read cargo config file")?;

    let config: toml::Value =
        toml::from_str(&config_content).context("Failed to parse cargo config as TOML")?;
                        ...
                        if let Some(flags_array) = rustflags.as_array() {
                            ...
                            let has_mold = flags_array.iter().any(|v| {
                                if let Some(s) = v.as_str() {
                                    let mold_arg =
                                        format!("link-arg=-fuse-ld={}", install_path.display());
                                    s.contains(&mold_arg)
                                } else {
                                    false
                                }
...
fn backup_cargo_config() -> AnyResult<Option<PathBuf>> {
    ...
    fs::copy(&config_path, &backup_path).context("Failed to create backup of cargo config")?;

    println!(
        "Backed up existing cargo config to: {}",
        backup_path.display()
    );
    Ok(Some(backup_path))
}
...
fn update_cargo_config() -> AnyResult<()> {
    ...
    let mut config: toml::Value = if config_path.exists() {
        ...
        toml::from_str(&content).context("Failed to parse existing cargo config")?
    } else {
    ...
    // Set linker
    linux_section.insert(
        "linker".to_string(),
        toml::Value::String("clang".to_string()),
    );
    ...
    fs::write(&config_path, updated_content).context("Failed to write updated cargo config")?;
...
fn remove_mold_from_cargo_config() -> AnyResult<bool> {
    ...
    let config_content = fs::read_to_string(&config_path).context("Failed to read cargo config")?;
