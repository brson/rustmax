...
pub fn install() -> AnyResult<()> {
    ...
    let install_path = get_cargo_bin_dir()?.join("mold");
...
fn get_installed_version() -> AnyResult<String> {
    let mold_path = get_cargo_bin_dir()?.join("mold");
...
fn update_cargo_config() -> AnyResult<()> {
    ...
    let rustflags = toml::Value::Array(vec![
        ...
        toml::Value::String("link-arg=-fuse-ld=mold".to_string()),
    ]);
